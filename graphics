...
def wait_key():
    ev = pygame.event.event_name(ev.unicode)
    if ev.type == KEYDOWN:
        if ev.key == K_ESCAPE or ev.key == K_SPACE :
            return ev.unicode

def wait_click_or_key(keys):
    continuer = True
    while (continuer):
        ev = pygame.event.wait()
        if ev.type == MOUSEBUTTONDOWN:
            ...
            if ev.key == K_ESCAPE or ev.key in keys:
                return ev.unicode
        continuer = (ev.type != MOUSEBUTTONDOWN and ev.type != KEYDOWN)
...
def ecrire(texte,point,taille,couleur,fenetre):
    font=pygame.font.SysFont('Times New Roman', round(taille))
